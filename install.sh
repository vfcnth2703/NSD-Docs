...
# С настройкой logrotate
# Ресурс ASCN_Volgograd (ftp) закомментирован
# ============================================
...
# Список всех возможных ресурсов (ftp закомментирован)
AVAILABLE_RESOURCES=(
    ...
    # "ASCN_Volgograd|/mnt/ftp|//ftp/ftp/ascnvolgograd"  # временно отключен
)
...
echo "Пример: 1 2 - только records и Synology"
...
echo -e "${BLUE}[1/7] Создание структуры директорий...${NC}"
sudo mkdir -p "$INSTALL_DIR" "$LOG_DIR" "$BIN_DIR" "$ETC_DIR"
sudo chown -R $(whoami):$(whoami) "$INSTALL_DIR"

# Шаг 2: Установка пакетов
echo -e "${BLUE}[2/7] Установка необходимых пакетов...${NC}"
sudo apt update -qq 2>/dev/null || true
sudo apt install -y cifs-utils

# Шаг 3: Создание точек монтирования
echo -e "${BLUE}[3/7] Создание точек монтирования...${NC}"
for item in "${SELECTED_RESOURCES[@]}"; do
    IFS='|' read -r name mount remote <<< "$item"
    if [ ! -d "$mount" ]; then
        sudo mkdir -p "$mount"
...
echo -e "${BLUE}[4/7] Создание скрипта мониторинга...${NC}"
...
echo -e "${BLUE}[5/7] Создание скрипта отправки писем...${NC}"
...
echo -e "${BLUE}[6/7] Создание конфигурации...${NC}"
...
# Шаг 7: Настройка logrotate
echo -e "${BLUE}[7/7] Настройка ротации логов (logrotate)...${NC}"
sudo tee /etc/logrotate.d/smb-healthcheck << 'LOGROTATE' > /dev/null
/opt/smb-healthcheck/logs/*.log {
    daily
    rotate 30
    compress
    delaycompress
    missingok
    notifempty
    create 0644 naumen naumen
    sharedscripts
    postrotate
        touch /opt/smb-healthcheck/logs/healthcheck.log
        touch /opt/smb-healthcheck/logs/send_alert.log
    endscript
}
LOGROTATE
...
# Установка sendemail и библиотек если нужны уведомления
if [ -n "$NOTIFY_EMAIL" ]; then
    echo -e "${BLUE}Установка библиотек для отправки писем...${NC}"
    sudo apt install -y sendemail libnet-ssleay-perl libio-socket-ssl-perl 2>/dev/null || true
fi
...
echo "  Настроена автоматически (ежедневно, хранится 30 дней)"
echo ""
